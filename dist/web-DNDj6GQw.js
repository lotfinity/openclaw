...
import { B as readWebSelfId, F as getWebAuthAgeMs, I as logWebSelfId, M as resolveWhatsAppAccount, P as WA_WEB_AUTH_DIR, W as webAuthExists, n as loadConfig, z as pickWebChannel } from "./config-CpsUWe0r.js";
import { Ct as resolveChannelGroupPolicy, Dt as resolveGroupSessionKey, J as loadSessionStore, Nt as saveMediaBuffer, Q as updateLastRoute, X as recordSessionMetaFromInbound, Y as readSessionUpdatedAt, wt as resolveChannelGroupRequireMention } from "./pi-embedded-helpers-LLoly8_-.js";
...
import { i as loadWebMedia, m as resolveTextChunkLimit, o as resolveMarkdownTableMode, p as resolveChunkMode, u as chunkMarkdownTextWithMode } from "./ir-DEs7zfF2.js";
import "./fetch-D1NTcRuw.js";
import { i as convertMarkdownTables, r as setActiveWebListener } from "./active-listener-Clx-80Pc.js";
import { d as registerUnhandledRejectionHandler } from "./runner-CaRQtuRR.js";
import { a as sendPollViaWaha, c as markdownToWhatsApp, i as sendMediaViaWaha, n as sendMessageWhatsApp, o as sendReactionViaWaha, r as sendReactionWhatsApp, s as sendTextViaWaha } from "./outbound-DgT7BiDr.js";
import { a as buildWahaWsCandidates, c as formatError, l as getStatusCode, o as connectWahaWebSocket, s as createWaSocket, u as waitForWaConnection } from "./login-qr-BW8ZHZSG.js";
import { t as loginWeb } from "./login-BQmuNwtk.js";
...
//#endregion
//#region src/web/inbound/waha-monitor.ts
function toNormalizedDirectId(raw) {
	const trimmed = raw.trim().toLowerCase();
	if (!trimmed) return null;
	if (trimmed.endsWith("@c.us")) return normalizeE164(trimmed.slice(0, -5));
	if (trimmed.endsWith("@s.whatsapp.net")) return normalizeE164(trimmed.slice(0, -15));
	return normalizeE164(trimmed);
}
function resolveChatKind(chatId) {
...
function normalizeTimestampMs(value) {
	...
	if (!parsed) return;
	if (parsed > 0xe8d4a51000) return parsed;
	return parsed * 1e3;
}
async function maybeLoadWahaMedia(payload, maxMb, apiKey) {
	...
	return {
		mediaPath: (await saveMediaBuffer(buffer, mimetype, "inbound", maxMb * 1024 * 1024, fileName)).path,
		mediaType: mimetype,
		mediaFileName: fileName
	};
}
async function monitorWahaInbox(options) {
	const account = resolveWhatsAppAccount({
		cfg: loadConfig(),
		accountId: options.accountId
	});
	...
	const apiKey = account.waha?.apiKey?.trim() || void 0;
	const session = account.waha?.session?.trim() || "default";
	const inboundLogger = getChildLogger({
		module: "waha-inbound",
	...
	ws.on("close", (code) => {
		resolveClose({
			status: code,
			isLoggedOut: false,
			error: `waha websocket closed (${code})`
		});
	});
	ws.on("message", async (data) => {
		let eventEnvelope = null;
		try {
			eventEnvelope = JSON.parse(String(data));
		} catch {
			return;
		}
		...
		if (eventEnvelope.event === "session.status") {
			...
			if (status && (status.toUpperCase() === "FAILED" || status.toUpperCase() === "STOPPED")) resolveClose({
				status: 499,
				isLoggedOut: false,
		...
		if (eventEnvelope.event !== "message") return;
		const payload = eventEnvelope.payload ?? {};
		const id = readString(payload.id) ?? readString(payload.id?._serialized);
		...
		const senderE164 = participant ? toNormalizedDirectId(participant) : isGroup ? null : toNormalizedDirectId(chatId);
		const from = isGroup ? chatId : toNormalizedDirectId(chatId) ?? chatId;
		if (id) {
			if (isRecentInboundMessage(`${account.accountId}:${chatId}:${id}`)) return;
		}
		const access = await checkInboundAccessControl({
			accountId: account.accountId,
			from,
			...
			connectedAtMs: Date.now() - 5e3,
			sock: { sendMessage: async (jid, content) => {
				const text = readString(content.text);
				if (text) await sendTextViaWaha(account, jid, text);
			} },
			remoteJid: chatId
		});
		...
		let media = null;
		try {
			if (Boolean(payload.hasMedia) || payload.media) media = await maybeLoadWahaMedia(payload, maxMb, apiKey);
		} catch (err) {
		...
		const sendMedia = async (content) => {
			if (content.text) {
				await sendTextViaWaha(account, chatId, String(content.text));
				return;
			}
			if (content.image) {
				const payload = content;
				await sendMediaViaWaha(account, chatId, payload.caption ?? "", payload.image, payload.mimetype ?? "image/jpeg", "image");
				return;
			}
			if (content.video) {
				const payload = content;
				await sendMediaViaWaha(account, chatId, payload.caption ?? "", payload.video, payload.mimetype ?? "video/mp4", "video");
				return;
			}
			if (content.audio) {
				const payload = content;
				await sendMediaViaWaha(account, chatId, "", payload.audio, payload.mimetype ?? "audio/ogg; codecs=opus", "audio");
				return;
			}
			if (content.document) {
				const payload = content;
				await sendMediaViaWaha(account, chatId, payload.caption ?? "", payload.document, payload.mimetype ?? "application/octet-stream", payload.fileName ?? "file");
			}
		};
		const inboundMessage = {
			id: id ?? void 0,
	...
	return {
		...
		sendMessage: async (to, text, mediaBuffer, mediaType, sendOptions) => {
			return { messageId: (mediaBuffer && mediaType ? await sendMediaViaWaha(account, to, text, mediaBuffer, mediaType, sendOptions?.fileName) : await sendTextViaWaha(account, to, text)).messageId };
		},
		sendPoll: async (to, poll) => {
			return { messageId: (await sendPollViaWaha(account, to, poll)).messageId };
		},
		sendReaction: async (chatJid, messageId, emoji) => {
			await sendReactionViaWaha(account, messageId, emoji);
		},
		sendComposingTo: async () => void 0
	};
}
...
async function monitorWebChannel(verbose, listenerFactory, keepAlive = true, replyResolver = getReplyFromConfig, runtime = defaultRuntime, abortSignal, tuning = {}) {
	...
	const effectiveListenerFactory = listenerFactory ?? (account.transport === "waha" ? monitorWahaInbox : monitorWebInbox);
	const cfg = {
		...baseCfg,
		channels: {
			...baseCfg.channels,
